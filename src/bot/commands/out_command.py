    ...
    try:
        ...
        if args:
            ...
            if mentioned_user:
                ...
                if admin_member.status not in [admin_member.ADMINISTRATOR, admin_member.OWNER]:
                    await update.message.reply_text("You must be a group admin or owner to remove others.")
                    return
        ...
        target_list = list_repo.get_by_trigger_name(session, group.id, target_list_trigger)
        if not target_list:
            await update.message.reply_text(f"List '{target_list_trigger}' not found.")
            return

        existing = user_list_repo.get_subscription(session, target_list.id, target_user_id)
        if not existing:
            await update.message.reply_text(f"User is not in the '{target_list.name}' list.")
            return

        user_list_repo.delete(session, existing.id)

        if mentioned_user:
            await update.message.reply_text(f"User manually removed from '{target_list.name}'.")
        else:
            await update.message.reply_text(f"You have been removed from '{target_list.name}'. To add yourself type /in {target_list_trigger}")

        LogService.add_log(session, user_id, group.id, "leave_list", f"Removed user {target_user_id} from list {target_list.name}")
