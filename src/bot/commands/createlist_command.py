...
async def createlist(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if not args or len(args) < 2:
            await update.message.reply_text("Usage: /createlist <trigger_name> <List Name>\nExample: /createlist devs Developers List")
            return
        ...
        banned_words = ["start", "help", "admin", "settings", "lists", "in", "out", "createlist", "deletelist"]
        if trigger_name in banned_words:
            await update.message.reply_text("This trigger name is not allowed as it conflicts with bot commands.")
            return
        ...
        if not group:
            await update.message.reply_text("Group not registered. Type /in everyone to initialize it first.")
            return

        existing = list_repo.get_by_trigger_name(session, group.id, trigger_name)
        if existing:
            await update.message.reply_text(f"A list with the trigger '{trigger_name}' already exists.")
            return

        new_list = TagList(group_id=group.id, name=name, trigger_name=trigger_name)
        list_repo.create(session, new_list)

        await update.message.reply_text(f"List '{name}' created successfully! Users can now type `/in {trigger_name}` to join.", parse_mode="Markdown")
        LogService.add_log(session, user_id, group.id, "create_list", f"Created list {name}")
